...
// Every op of the 61-layer stack (+ head) is priced at the peak rate of the
// dtype its recipe runs it in, forward + backward (2× forward) + whatever the
// recompute policy replays, and the pieces are SUMMED. Compute is conserved
...
export const SOL_DEFAULTS = { hw: 'h800', recipe: 'dsv3-fp8', recompute: 'dsv3', gpus: 2048, gbs: 15360, seq: 4096 };
...
// 14.8T tokens; GB200/GB300 = NVIDIA's published NeMo DeepSeek-V3 numbers
// (PP4/EP64 and PP2/EP32 at GBS 4096 × seq 4096). None published for H100.
export const REPORTED = {
  h800: { rate: 14.8e12 / (2.788e6 * 3600), src: 'implied by DeepSeek’s GPU-hours' },
  gb200: { rate: 4969, src: 'NVIDIA’s NeMo GB200 number' },
  gb300: { rate: 6338, src: 'NVIDIA’s NeMo GB300 number' },
};

const GROUP_OF = { attn: 'attn', qkv_down: 'mla', q_up: 'mla', kv_up: 'mla', o_proj: 'mla', router: 'router' };
const MM_KEY = { gate_up: 'ffn_gate_up' };
const FP8 = new Set(['e4m3', 'mxfp8', 'e5m6']);
export const rateOf = (hw, dt) => FP8.has(dt) ? hw.flops.fp8 : dt === 'fp32' ? hw.flops.fp32 : hw.flops.bf16;
// the dtype a node's kernel runs in (e5m6 names a stash format: the GEMM runs fp8)
const dtOf = (n, mm) => n.opKind === 'matmul' || n.opKind === 'attn' ? mm[MM_KEY[n.id] ?? n.id] ?? 'bf16' : 'bf16';

// FLOP/token per (pass, group), with the dtype each piece runs in
export function solFlops(cfg) {
  cfg = { ...SOL_DEFAULTS, ...cfg };
  const a = DSV3, mm = resolveMatmuls({ recipe: cfg.recipe });
  const marks = RECOMPUTE_PRESETS[cfg.recompute];
  const cells = {};   // `${pass}·${group}` → { flops, byDt: {dt: flops} }
  const add = (pass, group, dt, flops) => {
    if (!flops) return;
    const k = pass + '·' + group;
    const c = cells[k] ??= { pass, group, flops: 0, byDt: {} };
    c.flops += flops; c.byDt[dt] = (c.byDt[dt] ?? 0) + flops;
  };
  for (const [kind, count] of [['dense', a.denseLayers], ['moe', a.layers - a.denseLayers]]) {
    const nodes = blockGraph(kind, a, mm, cfg.seq);
    const ana = analyze(nodes, marks);
    for (const n of nodes) {
      if (!n.flopsTok) continue;
      const dt = dtOf(n, mm), f = n.flopsTok * count;
      ...
      for (const [g, frac] of pieces) {
        add('fwd', g, dt, f * frac);
        add('bwd', g, dt, 2 * f * frac);
        if (ana.replayed.has(n.id)) add('replay', g, dt, f * frac);
      }
    }
  }
  for (const op of HEAD_OPS(a)) {   // the head GEMM in the recipe's dtype; the loss is vector work, forward only
    const dt = op.cat === 'gemm' ? mm.lm_head : 'bf16';
    add('fwd', 'head', dt, op.ftok);
    add('bwd', 'head', dt, op.ftok * (op.bwdMult ?? 2));
  }
  return { cfg, cells: Object.values(cells) };
}

// seconds per step on one GPU: tokens per GPU × Σ FLOP/token ÷ peak rate of each piece's dtype
export function solStep(cfg) {
  ...
  for (const cell of cells) {
    cell.s = tokens * Object.entries(cell.byDt).reduce((t, [dt, f]) => t + f / rateOf(hw, dt), 0);
...
const RECIPES_UI = [['bf16', 'bf16'], ['dsv3-fp8', 'DeepSeek fp8'], ['all-fp8', 'all linears fp8'], ['nv-mxfp8', 'NVIDIA mxfp8']];
const RECOMP_UI = [['none', 'none'], ['dsv3', 'DeepSeek'], ['attn-replay', 'attention replay'], ['full', 'full']];
const HW_UI = [['h800', 'H800'], ['h100', 'H100'], ['gb200', 'GB200'], ['gb300', 'GB300']];
...
.so .ro { font-size: 11.5px; color: var(--c-52514e); min-height: 34px; margin-top: 4px; max-width: ${W}px; line-height: 1.45; }
...
class Dsv3Sol extends (typeof HTMLElement === 'undefined' ? class {} : HTMLElement) {
  ...
  _buildKnobs() {
    ...
    this._ui = {};
    const g1 = grp('GPU'); g1.append(row(seg('hw', HW_UI)));
    const g2 = grp('precision'); g2.append(row(seg('recipe', RECIPES_UI)));
    ...
    this._top.append(g1, g2, g3, g4, g5);
    this._syncKnobs();
  }
  _syncKnobs() {
  ...
  _layout() {
    ...
    // the untreated anchor: all-bf16 and nothing recomputed, same tokens
    const anchor = solStep({ ...this.cfg, recipe: 'bf16', recompute: 'none' }).total;
    ...
    for (const [i, p] of [...PASSES, { id: 'total', label: 'total' }].entries()) {
      ...
      for (const g of GROUPS) {
        ...
        if (s > 0) segs.push({ k: g.id, x0: px(cum), x1: px(cum + s), c: g.c, v: s, flops: cs.reduce((t, c) => t + c.flops, 0), byDt: cs.map((c) => c.byDt) });
        cum += s;
      }
  ...
  _draw(L) {
    ...
    // the untreated ghost on the total row: dashed overhang (all-bf16, no
    // recompute) past the bar, or a dashed tick where the total overshoots it
    const tr = L.rows[3];
    if (L.anchorX > L.totalX + 1)
      B.push(`<rect data-ghost x="${f1(L.totalX)}" y="${tr.y}" width="${f1(L.anchorX - L.totalX)}" height="${BARH}" fill="none" stroke="${C('#898781')}" stroke-dasharray="2 2"/>`);
    else if (L.anchorX < L.totalX - 1)
  ...
  _readout(L, row, seg) {
    ...
    const rateName = (dt) => FP8.has(dt) ? 'fp8' : dt;
    if (row && seg) {
      const r = L.rows.find((q) => q.id === row), s = r.segs.find((q) => q.k === seg);
      const g = GROUPS.find((q) => q.id === seg);
      const byDt = {};
      for (const m of s.byDt) for (const [dt, f] of Object.entries(m)) byDt[dt] = (byDt[dt] ?? 0) + f;
      const terms = Object.entries(byDt).map(([dt, f]) => `${fmtF(f)} at ${rateName(dt)} peak ${(rateOf(S.hw, dt) / 1e12).toLocaleString('en-US')} TFLOP/s`).join(' + ');
      this._ro.innerHTML = `<b>${g.label} · ${r.label}</b>: ${fmtF(s.flops)}/token = ${terms}; × ${tok} tokens = <b>${fmtS(s.v)}</b> (${(s.v / S.total * 100).toFixed(1)}% of the step).`;
      return;
    }
    if (row) {
    ...
    const ghost = L.anchor > S.total * 1.001 ? ` The dashed overhang is the same step at bf16 with nothing recomputed, ${fmtS(L.anchor)}.`
      : L.anchor < S.total * 0.999 ? ` The dashed tick is the same step at bf16 with nothing recomputed, ${fmtS(L.anchor)}.` : '';
    this._ro.innerHTML = `<b>${fmtS(S.total)} per step</b> = forward ${fmtS(S.rows.fwd)} + backward ${fmtS(S.rows.bwd)}` +
      ...
      `${fmtF(S.flopsTok)}/token, every op at its dtype’s peak, summed.${ghost}${rep}`;
  }
}
