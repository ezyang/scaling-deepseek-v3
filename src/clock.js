...
// scales the bytes and leaves the arithmetic alone.
import { HARDWARE } from './model.js';
import { C } from './theme.js';
import { knobCss } from './ui.js';

export const CLOCK_DEFAULTS = { hw: 'h800', dtype: 'bf16', op: 'mul' };
...
// the ops: tensors read, tensors written, one arithmetic op per number
export const OPS = {
  mul: { label: 'x × 2', reads: 1, writes: 1, what: 'multiplies', one: 'multiply' },
  add: { label: 'x + y', reads: 2, writes: 1, what: 'adds', one: 'add' },
};
// the tensor: one 4,096-token microbatch's hidden states
export const CLOCK_N = 4096 * 7168;
const US = 1e-6;

// every number the picture shows, for one config
export function clock(cfg) {
  cfg = { ...CLOCK_DEFAULTS, ...cfg };
  const hw = HARDWARE[cfg.hw], { b } = DTYPES[cfg.dtype], op = OPS[cfg.op];
  ...
  const bytesIn = op.reads * CLOCK_N * b, bytesOut = op.writes * CLOCK_N * b;
  const compute = CLOCK_N / opsRate, read = bytesIn / hw.hbm, write = bytesOut / hw.hbm;
  return { cfg, hw, b, op, N: CLOCK_N, opsRate, window, bytesIn, bytesOut, compute, read, write, mem: read + write, ratio: (read + write) / compute };
}
...
.ck .note { font-size: 11px; color: var(--c-52514e); max-width: 22ch; line-height: 1.35; }
`;
...
class Dsv3Clock extends (typeof HTMLElement === 'undefined' ? class {} : HTMLElement) {
  connectedCallback() {
    ...
    for (const k of Object.keys(CLOCK_DEFAULTS)) { const v = st?.[k] ?? this.getAttribute(k); if (v != null) this.cfg[k] = v; }
  ...
  _buildKnobs() {
    ...
    const g3 = grp('the kernel'); g3.append(row(seg('op', Object.entries(OPS).map(([k, o]) => [k, o.label]))));
    const g4 = grp('the tensor'); const note = el('div', 'note');
    note.innerHTML = `4,096 × 7,168 = <b>${fmtM(CLOCK_N)}</b> numbers — one 4,096-token microbatch’s hidden states`; g4.append(note);
  ...
  _syncKnobs() {
    ...
    for (const [k, eg] of Object.entries(this._ui))
      for (const b of eg.querySelectorAll('button')) b.classList.toggle('on', b.dataset.v == this.cfg[k]);
  }
  _set(k, v) {
  ...
  // dtype flip visibly halves the byte bars while the arithmetic bar holds
  _layout() {
    const K = clock(this.cfg);
    const worst = clock({ ...this.cfg, dtype: 'fp32', op: 'add' });
  ...
  _draw(L) {
    ...
    B.push(`<text ${dims} data-val="compute" x="${f1(Math.max(L.computeX, LAB + 1.5) + 5)}" y="${f1(y1 + BARH - 2)}">${fmtM(K.N)} ${K.op.what} · ${fmtUs(K.compute)}</text>`);
    ...
    B.push(`<text ${dims} data-val="mem" x="${f1(L.memX + 5)}" y="${f1(y2 + BARH - 2)}">read ${fmtB(K.bytesIn)} + write ${fmtB(K.bytesOut)} · ${fmtUs(K.mem)}</text>`);
    this._chart.innerHTML = `<svg width="${W}" height="${HB}" viewBox="0 0 ${W} ${HB}">${B.join('')}</svg>`;
    this._readout(K);
  }
  _readout(K) {
    const t = K.cfg.dtype;
    this._ro.innerHTML = `<b>${K.op.label}</b> over ${fmtM(K.N)} ${t} numbers: the ${K.op.what} are ${fmtM(K.N)} ÷ ${n(K.opsRate / 1e12, 1)} T/s = <b>${fmtUs(K.compute)}</b> of CUDA-core time ` +
      `(${n(K.hw.flops.fp32 / 1e12)} TFLOP/s counts an FMA as two FLOP; a lone ${K.op.one} fills one slot). ` +
      `Moving the numbers is ${K.op.reads} × ${fmtB(K.N * K.b)} in + ${fmtB(K.bytesOut)} out = ${fmtB(K.bytesIn + K.bytesOut)} ÷ ${n(K.hw.hbm / 1e12, 2)} TB/s = <b>${fmtUs(K.mem)}</b> of HBM time. ` +
