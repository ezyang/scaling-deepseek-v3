...
T.check('1024 GPUs: gate/up 6.1 GiB (expert-DP 1)', dims() === '6.1 GiB', dims());
stepBtn(0, +1).click(); await T.tick(600);
T.check('2048 GPUs again: 4.4 GiB', dims() === '4.4 GiB', dims());
...
T.check('dropdown jump EP4: 43.8 GiB', dims() === '43.8 GiB', dims());
epSel.value = '64'; epSel.dispatchEvent(new Event('change')); await T.tick(600);
T.check('dropdown back EP64: 4.4 GiB', dims() === '4.4 GiB', dims());
T.check('EP shows 64, PP shows 16', stpVal(3) === '64' && stpVal(1) === '16', `${stpVal(3)}/${stpVal(1)}`);
T.check('default = peak stage (1: early stage, 15 mb in flight)', sels()[0].selectedOptions[0].textContent.includes('L3–6') && sels()[0].selectedOptions[0].textContent.includes('peak'), sels()[0].selectedOptions[0].textContent);
const chart = () => layer().querySelector('.lv-bar')?.textContent ?? '';
T.check('chart row shows in-flight count', chart().includes('activations ×15mb'), chart().slice(0,120));
...
T.check('sched back to 1F1B', chart().includes('activations ×15mb'), '');
...
T.check('cum default: 4.4 GiB', dims() === '4.4 GiB', dims());
...
T.check('button ×4 blocks', cumBtn()?.textContent === '×4 blocks', cumBtn()?.textContent);
cumBtn().click(); await T.tick(60);
const mid = dia().querySelectorAll('rect[fill="#1baf7a"]').length;
await T.tick(600);
const end = dia().querySelectorAll('rect[fill="#1baf7a"]').length;
T.log('green mid/end', `${mid}/${end}`);
T.check('per block: 1.1 GiB', dims() === '1.1 GiB', dims());
T.check('tween passed through a mid state', mid > end && mid < 200, `${mid} vs ${end}`);
cumBtn().click(); await T.tick(600);
T.check('back to 4.4 GiB', dims() === '4.4 GiB', dims());

// EP stepper: 64 -> 32 (tweened)
stepBtn(3, -1).click(); await T.tick(600);
T.check('EP32: 7.0 GiB', dims() === '7.0 GiB', dims());
T.check('(8/rank)', [...dia().querySelectorAll('text.grplabel')].some(t => t.textContent.includes('(8/rank)')), '');
stepBtn(3, +1).click(); await T.tick(600);
T.check('EP64 again: 4.4 GiB', dims() === '4.4 GiB', dims());

// PP stepper: 16 -> 8 (jumps to the new peak: the last stage, 8 MoE blocks)
stepBtn(1, -1).click(); await T.tick(600);
T.log('PP8 dims', dims());
T.check('PP8 peak = stage 0 (4 moe): 3.5 GiB', dims() === '3.5 GiB', dims());
T.check('PP8 jumped to peak stage 0', sels()[0].selectedOptions[0].textContent.includes('peak') && sels()[0].selectedOptions[0].textContent.startsWith('0:'), sels()[0].selectedOptions[0].textContent);
stepBtn(1, +1).click(); await T.tick(600);
T.check('PP16 again: 4.4 GiB', dims() === '4.4 GiB', dims());
...
T.check('EP1: 169.8 GiB', dims() === '169.8 GiB', dims());
for (let i = 0; i < 6; i++) { stepBtn(3, +1).click(); await T.tick(250); }
await T.tick(600);
T.check('plus disabled at 64', stepBtn(3, +1).disabled, '');
T.check('restored: 4.4 GiB', dims() === '4.4 GiB', dims());

// plan stage map + legend
const plan = layer().closest('.anat-grid')?.querySelector('dsv3-anatomy-plan');
T.check('plan stage map (stage 1: both vocab rows gated)', plan.textContent.includes('MoE block ×4') && plan.textContent.includes('(stage 0 only)') && plan.textContent.includes('(last stage only)'), '');
...
T.check('absolute values at bar ends, no capacity factors', chart().includes('116.5 GiB') && !chart().includes('×1/'), chart().slice(-160));
...
T.check('total label stays full (116.5 GiB)', chart().includes('116.5 GiB'), '');
...
T.check('solo weights: box shows weights only', dims() === '896.0 MiB', dims());
md(rowLabels()[0]); await T.tick(600);
