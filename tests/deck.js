...
next(); await T.tick(1000);
T.check('step 3: hypothetical ZeRO-2 — dashed card, tag in the nav row', L().zero === 2
...
T.check('chart y is slide-invariant', Math.abs(L().querySelector('.lv-bar').getBoundingClientRect().top
  - deckBarY) < 1, '');
next(); await T.tick(1000);
T.check('step 4: EP64 — the hypothetical reverted, solid card', L().zero === 1 && L().ep === 64
  && getComputedStyle(L().querySelector('.lv')).borderTopStyle === 'solid', `zero ${L().zero}`);
T.check('step 4 baseline is ZeRO-1 (skips the hypothetical)',
