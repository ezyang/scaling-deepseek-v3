...
// the strip widget: DPV segment present, placeholder note while rendering is deferred
...
T.check('strip shows the deferred-rendering note', w.textContent.includes('TBD'), '');
T.check('strip note states the chunk count', w.textContent.includes(`${2 * l().pp + 1} half-rank chunks`), '');
