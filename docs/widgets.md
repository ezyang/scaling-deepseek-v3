...
| `optim` | view | boolean | param-bytes only: stack green optimizer-state squares (8 B/param — fp32 master + two bf16 moments; memory-bars optimizer color) under the blue weight squares, same unit, so the 4:1 ratio is the picture; pinned per block (no cumulative toggle); the plan strips follow. The legend entries are visibility checkboxes (URL state `showWeights`/`showOptim`): every number totals exactly the squares shown, and a hidden run keeps its blank cells so nothing shifts |
