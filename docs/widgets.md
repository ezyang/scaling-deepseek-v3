...
| `squares` | view | absent · boolean · `matrix` | `lens="params"` only: draw the parameter COUNT as squares — boolean = one square per routed expert (44.0M), `matrix` = one per 7168×2048 expert matrix (14.7M; 01 uses this: routed gate/up = 512, down = 256, shared 2 + 1, whole-count floors, sub-square ops a hollow trace). Active params are the weights blue, inactive grey (`INACTIVE` in localmodel.js: the 248 unfired routed experts, drawn after the active ones, and the embedding table, per the 37B accounting) in BOTH tally views; the tally's active view turns the inactive squares hollow (resident, not counted) — box heights never change. The head legend names both plus the unit |
...
| `xlayers`, `xinflight` | state | numbers (61, 1) | combined-view multipliers (× layers × in-flight × 4096 tokens). 02's AC/fp8 instances pin the EP64·PP8·DualPipeV context: `xlayers="8" xinflight="8.5"` (an interior rank's 8 MoE layers × the PP+½ law), so the stash readout tallies byte-exact with the fit chart/deck. `xtag` (text) is appended to the readout in parens to name that context. `ctx` (JSON `{world, pp, ep, zero}` — plus `sched:"interleaved", vpp, hw, stage, peak:false` for the Megatron family, which renders PP · VP · rank rN · 'interleaved 1F1B' and the GPU chip) renders the section's FIXED parallelism as locked knob chips in the house style — readouts, not levers |
...
| `world`, `pp`, `ep`, `tp`, `tps`, `vpp`, `sched`, `fold`, `layout`, `a2a`, `grads`, `fp8params`, `hw`, `hws`, `facs`, `recomputes` | state / view | numbers · `1f1b`/`interleaved`/`one` · `reflect`/`wrap` · layout string · HARDWARE key · comma list · `2:PP,32:EP,256:GPUs` | the local lens's PAGE-SET defaults (03 adds them; 02 relies on `LOCAL_PAR` = 2048 GPUs · PP8 · EP64 · DualPipeV). URL state overrides. `sched="interleaved"` selects the **Megatron family**: Megatron's interleaved 1F1B with `vpp` virtual stages per rank on the wrap fold (chunk *i* on rank *i* mod PP), a `layout` string in Megatron-Core's `--pipeline-model-parallel-layout` grammar (`E` embedding · `t` layer · `m` MTP · `L` loss · `|` ends a chunk · `X*N` / `(…)*N` repeat; default `megatronLayout(pp, vpp)`: the 64 slots E · 61 t · m · L dealt evenly, front chunks taking remainders) replacing the 63-slot split, PP over {1,2,4,8,16}, a VP stepper, a plain rank stepper (every rank differs), and the schedule segment reading 'interleaved 1F1B / ×1 mb'. The law (`ilvLive`/`ilvPeak`): the rank's static program (schedules.py — forwards chunk-major in groups of PP, warmup 2(PP−r−1) + (VP−1)·PP chunk-forwards, +1 with the 1F1B all-to-all overlap `a2a` (on in NVIDIA's MXFP8 recipes), then strict 1F1B) is replayed and the live stashes tracked per chunk after every op — a stash is born at the rank's own F and dies at its own B, so no cross-rank timing is needed; the count peak is PP·VP + PP − 2r (−1 without a2a) chunk-stashes, but the cells charge the BYTE peak: the op after which Σ live_c × (chunk c's MoE layers × aM + dense × aD) is largest (a light last chunk — 1 layer + MTP + loss — puts it at a different moment), and each layer KIND gets its mean over its own layers at that moment (`inflightOf(…, layout, kind, {a2a})`; the fit chart labels the cells' P6; the sheet carries P6 = MoE, P6d = dense, P8 = VP, P9 = rank, P10 = the chunk-stash law, and every activation formula reads (L1 × P6) · rate + (L2 × P6d) · rate instead of 02's trailing × P6 — 02's formula strings are untouched). `grads="bf16"` (config `gradB: 2`) prices the gradient buffer at 2 B/param (Megatron's perf recipes set grad_reduce_in_fp32=False; 02's fp32 stays the default) — legend, cells G• and labels follow; `fp8params` (config `fp8Params`) seeds the fp8-resident-params flag, whose label reads 'mxfp8 params (row + col)' on the MX recipe (TE keeps both orientations resident after the param all-gather — the same 2 × (1 + 1/32) B/param as Hopper's e4m3+ᵀ; the cells' F1 formula says `2 × 1/32` there); `tp` (config key `tp`, default 1; `tps="1,2,4,8"` puts a TP stepper in the SPMD-mesh row) is TENSOR PARALLELISM as Megatron lays DeepSeek-V3 out (sequence parallel on, expert-tensor-parallel 1): non-expert DP becomes GPUs ÷ PP ÷ TP (cell P4 = P1 / (P2 × P11)), expert-DP stays GPUs ÷ PP ÷ EP (P5 = P4 × P11 / P3 — TP only widens the experts' replication group); weights split three ways — class `d` (q/kv up-proj, attn out-proj, shared expert, dense FFN) and `v` (embedding, lm head) shard 1/TP, class `r` (`TP_REPLICATED`: norms, router, the MLA down-projections — TELinear 'duplicated') stays whole on every TP rank with an optimizer shard over DP that is NOT de-duplicated across TP, class `e` is EP's business (cells N5–N8 = the sharded/replicated parts of a MoE/dense layer; Q2 = L1 × (N5 / P11 + N6) + L2 × (N7 / P11 + N8); Q3 = L3 × N4 / P11 + L5 × H1); and EVERY activation stash divides by TP (sequence parallel shards the residual/MoE path by tokens, attention by heads, the head by vocab — same bytes), carried as this GPU's token share: P7 = 4096 / P11 and the chips'/readouts' `TOK` (`_tok()`; `ctx.tp` for the ctx'd tiers). Sanity replays 192 configs incl. TP2 against independent shard math. Not modeled: the shared expert's full-sequence fc1 stash under `moe_shared_expert_overlap` (off in the flex-dispatcher configs), TP communication buffers. `recomputes="none,moe_act,…"` curates the recompute chips (Megatron's `--recompute-modules` are presets: `moe_act`, `mla_up_proj`, `layernorm`, `core_attn`, `moe`; `mlp` is dense-only and not a preset). `schedGeom` derives vpp/fold/layout from a config: DualPipeV ignores authored vpp/fold; interleaved (or any config on the wrap fold, e.g. its ×1 mb) keeps them. `hw` picks the capacity yardstick (HARDWARE.memGB: h100 80 · gb200 184 · gb300 276 GiB — what PyTorch reports, rounded down); `hws="gb200,gb300"` makes it a knob in the cluster group; the quant tiers read `ctx.hw` for their red line (px/GiB shrinks to 2 above 100 GiB). `facs` sets the fit chart's "a span is a factor" ticks (absent = 02's ×8 PP · ×64 EP · ×2048 GPUs). Authored configs (`from`/`to`/deck steps) accept `hw`, `sched`, `vpp`, `fold`, `layout` |
...
| *(forwarded)* | | `controls recipe recipes recompute detail transposed for nocaption kind xlayers xinflight xtag ctx lens strips nostrips optim consolidated local cumulative world pp vpp ep sched fold layout hw hws facs` pass through to the inner layer |
...
## `<dsv3-pp-fold ep=… view=… pp=… vpp=… fold=… layout=…>` — the pipeline fold stage map

How a pipeline fold distributes PARAMETERS over ranks (02's PP section:
DualPipeV's V; 03's: Megatron layouts on the wrap fold). `pp`/`vpp` set the
geometry (default 8 × 2), `fold` = `reflect` (chunk v pairs with D−1−v on
rank min(v, D−1−v)) or `wrap` (chunk v on rank v mod PP; a `layout` string
implies it and replaces the slot split — an `m` chunk adds a dashed MTP slot
to the strip and a 'mtp' range label, carrying no params: MTP is outside the
model). Later-pass chunks fly to their rank's row and dock after the bars
already there; `_rankChunks(r)` and `data-ranktotal` carry exact values.
The 02 description follows.
...
single-microbatch wave. Under the Megatron family (sched `interleaved`, m % PP = 0) the strip draws
Megatron-Core's interleaved 1F1B program (`_officialInterleaved`: forwards
chunk-major in groups of PP microbatches, warmup 2(PP−r−1) + (VP−1)·PP + 1,
then strict 1F1B, then cooldown; VP1 = the plain schedule; 'auto' draws
PP·(VP+2) microbatches; beyond four chunks neighbouring chunks share a shade)
and its braid peaks at exactly PP·VP + PP − 2r chunks (tests/ilv.js).
Otherwise the strip draws DualPipeV: with m ≥ 2·PP
...
direction — the data-sheet numbers the whole post runs on. Rows = forward /
backward / recompute / total, each a stack by op group (attention core ·
MLA projections · routed experts · shared expert · dense FFN · router ·
norms/RoPE/SwiGLU · lm head/loss); a linear seconds axis that autoscales to
...
| attr | kind | values | meaning |
|---|---|---|---|
| `hw` | state | `h800` · `h100` | the tech sheet + the reported line |
...
| state | | `#s:<id>={hw,recompute,gpus,gbs,seq}` | URL hash, when the element has an id |

Readout (`.ro`, height reserved): at rest the sum and its terms; row hover
the row's pieces; segment hover the piece's FLOP/token × tokens ÷ peak and
share. `solStep(cfg)` / `solFlops(cfg)` are node-importable (goldens
