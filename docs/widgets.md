...
| `optim` / `consolidated` | view | boolean | param-bytes only: stack byte-component squares under the blue weight squares, in the memory-bars stacked order with the memory-bars segment colors (`BYTE_COMPS`: weights #2a78d6 2 B/param · fp32 gradients #eb6834 4 B/param · optimizer states #1baf7a 8 B/param). `optim` = weights + optimizer; `consolidated` = all three + an aggregate amber saved-activations band at the block bottom (`ana.savedBytes` × 4096 tokens — activations live on wires, not in weight matrices, so ONE strip, no per-op squares; pair with `recipe="bf16"` so stashes price at 2 B). Same global unit, so the ratios are the picture; pinned per block. The legend entries are visibility checkboxes (URL state `showWeights`/`showGrads`/`showOptim`/`showActs`): numbers always total exactly the squares shown (nothing visible = no number), and toggling animates per-block-tween style — the color's squares pour in/out and boxes/band reflow smoothly (numbers snap) |
| `local` | view | boolean | param-bytes only, implies `consolidated`: what ONE GPU holds under the fiat parallelism (`LOCAL_PAR`: 2048 GPUs, PP16 over a contiguous floor split of the 61 layers, ZeRO-1) with EP (32/64) and PP-stage (0–15) selects in the mini head (URL state `ep`/`stage`). Expert weights divide by EP; optimizer states shard over each parameter's replication group (dense /DP=128, expert /expert-DP=world/pp/EP — why expert green squares survive); the kind and ×N follow the stage (0 = embedding + 3 dense; 15 = + final norm/lm head; the plan doubles as the stage map, gating those rows); activations = one microbatch per stage layer (schedule in-flight deferred). Fixed 448 MiB unit |
...
| *(forwarded)* | | `controls recipe recompute detail transposed for nocaption kind xlayers xinflight lens strips optim` pass through to the inner layer |
