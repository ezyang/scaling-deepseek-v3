...
## `<dsv3-clock hw=… dtype=… op=…>` — one microsecond on each meter (src/clock.js, post 03 draft)
...
storage dtype. Below it, one elementwise kernel over one 4,096 × 7,168
tensor (a microbatch's hidden states) on a µs axis: the CUDA-cores row is
its arithmetic (N ÷ half the fp32 peak — an FMA counts as two FLOP, a lone
multiply or add fills one slot); the HBM row is its bytes, the reads then
the write end to end (one pipe), in the dtype's precision-family color.
The axis is FIXED at the worst case on it (fp32, the two-input op) so a
dtype flip visibly halves the byte bars while the arithmetic bar holds; the
shaded band at the origin is the 1 µs window.

| attr | kind | values | meaning |
|---|---|---|---|
| `hw` | attr | `h800` (default) · `h100` | fixed; no knob |
| `dtype` | state | `fp32` · `bf16` · `fp8` | bytes per number (4 · 2 · 1) |
| `op` | state | `mul` (x × 2: read 1, write 1) · `add` (x + y: read 2, write 1) | the kernel |
| state | | `#c:<id>={hw,dtype,op}` | URL hash, when the element has an id |
